    ...
    <section id="main-content">
        ...
        <%
            ...
            try {
                ...
                if (rs.next()) {
        %>
            <form action="adminPage_updateComment_pro.jsp" method="post">
                <input type="hidden" name="no" value="<%= rs.getInt("no") %>">
                <label>게시글 번호:</label><input type="text" name="room_no" value="<%= rs.getInt("room_no") %>" readonly><br>
                <label>내용:</label><input type="text" name="content" value="<%= rs.getString("content") %>"><br>
                <input type="submit" value="수정">
            </form>
        <%
                } else {
                    out.println("<p>채팅 정보를 찾을 수 없습니다.</p>");
                }
            } catch (Exception e) {
