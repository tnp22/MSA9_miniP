...
<head>
    ...
    <script>
        function showAlertAndRedirect(message) {
            alert(message);
            window.location.href = 'adminPage_postList.jsp'; 
        }
    </script>
</head>
<body>
...
<%
    ...
    if (postId != null) {
        try {
            ...
            if (result > 0) {
                out.println("<script>showAlertAndRedirect('게시글 삭제 성공!');</script>");
            } else {
                out.println("<script>showAlertAndRedirect('게시글 삭제 실패!');</script>");
            }
        } catch (Exception e) {
